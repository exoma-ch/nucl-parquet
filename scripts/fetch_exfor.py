...
Usage:
    ...
    # Fetch neutron data for Li with explicit masses:
    python scripts/fetch_exfor.py --projectile n --element Li --masses 0 6 7
...
def fetch_element(
    ...
    output_dir: Path,
    session: requests.Session,
    extra_masses: list[int] | None = None,
) -> int:
    ...
    # Merge in any extra masses provided on CLI
    if extra_masses:
        for m in extra_masses:
            if m not in target_masses:
                target_masses.append(m)

    all_rows: list[dict] = []

    for mass in sorted(set(target_masses)):
...
def main() -> None:
    ...
    parser.add_argument(
        "--projectile", choices=list(PROJECTILE_MAP.keys()),
        help="Projectile type (default: all if --all)",
    )
    parser.add_argument(
        "--element", help="Element symbol (e.g., Cu, Fe, Li)",
    )
    parser.add_argument(
        "--masses", type=int, nargs="+",
        help="Explicit target mass numbers to query (e.g. --masses 0 6 7). "
             "Added on top of any masses found in TENDL.",
    )
    parser.add_argument(
    ...
    for proj in projectiles:
        ...
        for elem in elements:
            rows = fetch_element(elem, proj, args.output, session, extra_masses=args.masses)
            total_rows += rows
