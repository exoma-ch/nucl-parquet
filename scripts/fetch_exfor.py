...
def main() -> None:
    ...
    parser.add_argument(
        "--projectile",
        choices=list(PROJECTILE_MAP.keys()),
        help="Projectile type (default: all if --all)",
    )
    parser.add_argument(
        "--element",
        help="Element symbol (e.g., Cu, Fe, Li)",
    )
    parser.add_argument(
        "--masses",
        type=int,
        nargs="+",
        help="Explicit target mass numbers to query (e.g. --masses 0 6 7). Added on top of any masses found in TENDL.",
    )
    parser.add_argument(
        "--all",
        action="store_true",
